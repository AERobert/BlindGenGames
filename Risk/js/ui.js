...
import { TERRITORIES, CONTINENTS, NAV_KEYS, findTerritory } from './data.js';
import { G, gameLog, getPlayerTerritories, getEnemyNeighbors, getControlledContinents, currentTerritory, currentPlayer, areConnected } from './state.js';
...
export function isQuickJumpActive() { return quickJumpActive; }
export function isTroopInputActive() { return troopInputActive; }
export function isDiceActive() { return diceActive; }
export function isReportActive() { return reportActive; }
...
export function updateQuickJumpMatches(query) {
...
export function cycleQuickJumpMatch(direction) {
  if (quickJumpMatches.length === 0) return;
  const delta = direction === 'up' ? -1 : 1;
  setQuickJumpSelection(quickJumpIndex + delta);
}

export function confirmQuickJumpSelection() {
  if (quickJumpMatches.length === 0) return;
...
export function showTroopInput(title, desc, min, max, callback) {
...
export function confirmTroopInput() {
  const input = document.getElementById('troop-input');
...
export function cancelTroopInput() {
  document.getElementById('troop-input-overlay').classList.add('hidden');
  troopInputActive = false; troopCallback = null;
  speech.speak('Cancelled.');
}

export function showDiceResult(result, callback) {
...
export function showAfterActionReport(winnerId) {
  reportActive = true;
...
export function hideReport() { reportActive = false; document.getElementById('report-overlay').classList.add('hidden'); }
export function showHelp() { document.getElementById('help-overlay').classList.remove('hidden'); }
export function hideHelp() { document.getElementById('help-overlay').classList.add('hidden'); }

// ============== ANNOUNCEMENTS ==============

export function announceTerritory() {
  const t = currentTerritory();
  if (!t) return;
  const ter = G.territories[t.name];
  const owner = ter.owner !== null ? G.players[ter.owner] : null;
...
export function announcePlayerStatus(idx) {
